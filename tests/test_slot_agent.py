...
def _llm_returning(scenario_id, slots) -> MagicMock:
    """with_structured_output(...).invoke(...)가 include_raw 형식 dict를 반환하도록 모킹.

    구조화 출력은 {"raw", "parsed", "parsing_error"} dict를 반환하며,
    parsed는 검증된 SlotExtraction 인스턴스다.
    """
    mock = MagicMock()
    mock.with_structured_output.return_value.invoke.return_value = {
        "raw": MagicMock(),
        "parsed": SlotExtraction(scenario_id=scenario_id, slots=slots),
        "parsing_error": None,
    }
    return mock
...
class TestAskFlow:
    ...
    def test_empty_slots_asks_first_with_candidates(self, mock_model) -> None:
        mock_model.return_value = _llm_returning("regional_sales", {})
    ...
    def test_parent_filled_asks_child_filtered(self, mock_model) -> None:
        mock_model.return_value = _llm_returning("regional_sales", {"country": "한국"})
...
class TestFetchFlow:
    ...
    def test_all_slots_filled_runs_metric(self, mock_model) -> None:
        mock_model.return_value = _llm_returning(
            "regional_sales", {"country": "한국", "branch": "강남점"}
        )
...
class TestNoMatch:
    ...
    def test_null_scenario_returns_no_match(self, mock_model) -> None:
        mock_model.return_value = _llm_returning(None, {})
        content = slot_wrapper(_state("안녕"))["messages"][-1].content
        assert "지역별 매출" in content  # 카탈로그 안내

    @patch("src.slot_agent.agent.get_chat_model")
    def test_structured_parse_failure_falls_back_to_no_match(self, mock_model) -> None:
        # 구조화 출력 파싱 실패(parsed=None) → no_match로 안전 폴백
        mock = MagicMock()
        mock.with_structured_output.return_value.invoke.return_value = {
            "raw": MagicMock(),
            "parsed": None,
    ...
    def test_unknown_scenario_id_falls_back(self, mock_model) -> None:
        mock_model.return_value = _llm_returning("does_not_exist", {})
...
class TestFreeSlot:
    ...
    def test_missing_free_month_asks_for_value(self, mock_model) -> None:
        mock_model.return_value = _llm_returning("new_signups", {"channel": "검색광고"})
